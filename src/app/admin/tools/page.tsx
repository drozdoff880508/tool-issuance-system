            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  tools.map((tool) => {
                    const available = tool.availableQuantity ?? tool.quantity - (tool.issuedQuantity || 0)
                    const isFullyIssued = available <= 0
                    ...
                    return (
                      <TableRow key={tool.id} className={tool.status === 'WRITTEN_OFF' ? 'opacity-50' : ''}>
                        ...
                        <TableCell className="text-center">
                          <span className={`font-bold ${tool.issuedQuantity ? 'text-orange-600' : ''}`}>
                            {tool.issuedQuantity || 0}
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
                          <Badge variant={isFullyIssued ? 'destructive' : 'outline'} className={isFullyIssued ? '' : 'bg-green-50 text-green-700 border-green-200'}>
                            {available}
                          </Badge>
                        </TableCell>
                        <TableCell>
